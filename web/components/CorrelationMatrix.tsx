    ...
    <>
      ...
      {!!outliers?.length && (
        <section id="dataRelOutliers" className={styles.cardSection}>
          <h2 className={styles.h2}>
            이상치{" "}
            <InfoTip text="반 친구들의 키를 한 줄로 세운다고 생각해보세요. 대부분은 비슷비슷한 키에 모여있지만, 어쩌다 한두 명은 유난히 크거나 작을 수 있어요. 이렇게 '대부분과 아주 다른 값'을 이상치라고 불러요. 이 그림에서 상자는 친구들이 옹기종기 모여있는 가운데 구간(전체의 중간 50%)을 보여주고, 가운데 세로선은 딱 중간값이에요. 점으로 찍힌 건 그 무리에서 크게 벗어난 이상치예요. 맨 오른쪽 숫자는 그 컬럼의 전체 값 중에서 이상치가 차지하는 비율이에요 — 높을수록 유난히 튀는 값이 많다는 뜻이에요." />
          </h2>
