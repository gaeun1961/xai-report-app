...
const STEPS = [
  ...
  { title: "맞힐 컬럼 고르기", desc: "결과가 담긴 컬럼(예: 생존 여부)을 하나 골라요." },
  { title: "판단 근거 읽기", desc: "어떤 정보가 예측을 얼마나 밀었는지 문장으로 봐요." },
];

const FACTORS = [
  { name: "성별", width: "100%", value: "+0.22" },
  { name: "요금", width: "68%", value: "+0.15" },
  { name: "좌석 등급", width: "50%", value: "+0.11" },
];
      ...
      <section className={styles.section}>
        ...
        <div className={styles.window}>
          ...
          <div className={styles.windowBody}>
            ...
            <div className={styles.pane}>
              <span className={styles.paneLabel}>리포트가 보여주는 것</span>
                ...
                {FACTORS.map((f) => (
                  <div key={f.name} className={styles.barRow}>
                    ...
                    <span className={styles.barTrack}>
                      <i className={styles.barFill} style={{ width: f.width }} />
                    </span>
                    <span className={styles.barVal}>{f.value}</span>
                  </div>
                ))}
              </div>
            </div>
